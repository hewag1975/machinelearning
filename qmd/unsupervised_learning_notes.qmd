...
centroids = np.array([[0.2, 0.2, 0.2, 0.2], [0.6, 0.6, 0.6, 0.6]])
...
* normalize features to zero mean
* choose an axis to capture major share of data variance (rotate around 0)
* continue with axis 2, 3, ... on remaining variance (perpendicular)
* other applications of PCA in the past:
  * data compression 
  * speedup training of supervised learning

Steps:

* feature scaling
* fit data to PCA to obtain axes $z_1, z_2, ....$
* in scikit-learn, mean normalization is included in fit-step
* examine variance explained per component $z_i$
* transform $x_1, x_2, ...$ to the new axes $z_1, z_2, ...$


## Reinforcement learning

* example: move control sticks given position of a helicopter
* position: state $s$
* move sticks: action $a$
* supervised learning does not work well here
* instead define a reward function, positive reward for desired action, 
negative reward for non-desired action
* tuple of (state, action, reward, next state): $(s, a, R(s), s`)$
* reward is weighted by discount factor $\gamma$ to account for number of steps 
required to get reward
* discount factor is like interest rate
* policy $\pi$ to map from a state to an action
* task of reinforcement learning is to define $\pi$ in such a way that it 
recommends an action $a$ in every state to maximize return $R$.
* formalism of reinforcement learning is called Markov Decision Process (MDP), 
i.e. the future only depends on the current state

State-action-value function ($Q$-function):

* $Q(s,a)$ provides return if you are in state $s$, take action $a$, behave 
optimally afterwards
* best possible return from state $s$ is $max_aQ(s,a)$
* Bellman equation: $Q(s,a)=R(s)+\gamma*max_{a'}Q(s',a')$
* Bellman equation in terminal state: $Q(s,a)=R(s)$
* stochastic environments, calculate expected return, i.e. average of * $Q(s,a)$ 
in case of many iterations
* replay buffer: store latest 10,000 tuples

Continuous state spaces:

* so far only discrete states
* in many applications state $s$ is described by multiple parameters, i.e. by a 
vector of numbers (e.g. car, helicopter)

Deep reinforcement learning (aka deep Q-learning):

* input: state-action-pair
* output: state-value-function for all actions, pick action that maximizes $Q(s,a)$
* formals: $x=Q(s,a); y=R(s)+\gamma*max_{a'}Q(s',a')$
* objective of NN is to map $Q(s,a)$ to $y$
* training set: create tuples of many state-action-combinations
* improved architecture: input is only state, output is state-action-value equation 
for all actions
* $\epsilon$-greedy-policy: 
  * approach to choose actions while still learning
  * in some state $s$ with a prob of 0.95 pick the action that maximizes $Q(s,a)$
  * in some state $s$ with a prob of 0.05 ($\epsilon$) pick an action randomly
  * background is that while learning even the actions that do not maximize $Q(s,a)$ 
  must be explored

Mini batches:

* batch: use full training data $m$ set during model training
* in case of large training set, using a subset $m'$ may speed up training
* mini batch approach uses a different subset per iteration

Soft update:

* example linear regression, instead of updating $w, b$ after every iteration, set
  * $w=0.01*w_{new}+0.99*w$
  * $b=0.01*b_{new}+0.99*b$
* convergence of reinforcement algorithm is more reliable with soft update

State of reinforcement learning:

* RI algorithms are much easier to implement in a simulation environment than in 
a real robot
* fewer RI applications than supervised and unsupervised learning
